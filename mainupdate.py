...
def parse_content(content):
    ...
    for element in content['body']['content']:
        if 'paragraph' in element:
            ...
            orange_texts = [elem['textRun']['content'].strip() for elem in paragraph['elements'] if 'textRun' in elem and elem['textRun']['textStyle'].get('foregroundColor') == {'color': {'rgbColor': {'red': 1.0, 'green': 0.54901963, 'blue': 0.0}}}]
            ...
            if text.startswith("Sub-Topic"):
                ...
                if subtopic:
                    if screen and (is_mcqss or is_true_false):
                        if options and orange_texts:
                            correct_option = next((opt for opt in options if opt in orange_texts), None)
                            answer_index = options.index(correct_option) if correct_option else -1
                        else:
                            answer_index = -1
...
def cli_startup():
    tprint("Doc2JSON")
    print(Fore.CYAN + "Developed by: Your Name\n")
